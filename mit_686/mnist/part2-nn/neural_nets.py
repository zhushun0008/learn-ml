...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        hidden_layer_error = self.hidden_to_output_weights.T * output_layer_error

        bias_gradients = np.multiply(hidden_layer_error, np.vectorize(rectified_linear_unit_derivative)(
            hidden_layer_weighted_input))
        hidden_to_output_weight_gradients = np.matmul(output_layer_error, hidden_layer_activation.T)
        input_to_hidden_weight_gradients = np.matmul(bias_gradients, input_values.T)
